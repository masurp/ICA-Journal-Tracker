...
Usage:
  ...
  python3 fetch_data.py --force          # skip DOI cache, re-fetch all LLM keyword data
"""
...
def load_doi_cache(journal_id: str) -> dict[str, dict]:
    """Load existing enrichment data (topics) keyed by DOI."""
            ...
            for paper in section:
                ...
                if doi:
                    cache[doi] = {
                        ...
                        "topics_llm": paper.get("topics_llm", False),
                    }
        return cache
    except Exception:
        return {}
...
def enrich_with_llm(papers: list[dict], doi_cache: dict[str, dict]) -> None:
    ...
    for paper in papers:
        doi = paper["doi"].lower()
        # Hit local cache first — only trust if previously generated by LLM
        if doi in doi_cache and doi_cache[doi]["topics"] and doi_cache[doi].get("topics_llm"):
            paper["topics"] = doi_cache[doi]["topics"]
            paper["topics_llm"] = True
        ...
        prompt = (
            "Generate 3-4 concise topic keywords for this communication science paper. "
            "Return ONLY a JSON array of strings, e.g. [\"Social media\", \"Political communication\"]. "
        ...
        try:
            ...
            # Extract JSON array robustly — handles markdown code fences or extra text
            match = re.search(r'\[.*?\]', raw, re.DOTALL)
            topics = json.loads(match.group()) if match else []
            topics = [t for t in topics if isinstance(t, str)][:4]
        except Exception as exc:
            errors.append(f"LLM enrichment for {doi}: {exc}")
            topics = []
...
def main():
    ...
    parser.add_argument(
        ...
        help="Ignore DOI cache — re-call Claude for all papers from scratch.",
    )
